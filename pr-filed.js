        ...
        if (result.cmd_data.propagation_status !== "no") {
            for (let prop_branch of result.cmd_data.propagation_branches) {
                if (prop_branch.date === null) {
                    ...
                    messages.push(`Will cherry pick the commits to ${branch_desc} (${s2l_branch_map[branch_desc]}) on ${prop_branch.date.getFullYear()}-${prop_branch.date.getMonth()+1}-${prop_branch.date.getDate()}.`)
                }
            }
        }
